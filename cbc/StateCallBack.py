...
class CallBack:
	...
	def _bizlogic(self, message_body):
		#message_body
		logic_body = '[' + self.process_idle.name + ', ' + str(self.process_idle.pid) + ', RabbitMQ Cluster : ' + str(self.rabbitmq_cluster) + ', Redis : ' + str(self.redis_server) + ', Time : ' + str( datetime.datetime.now().time() ) + ', State Callback]\n' + str(message_body)
