...
class Channel(object):
    ...
    def basic_qos(self, prefetch_size=0, prefetch_count=0, _global=False):
        return self.connection._basic_qos(self.channel_id, prefetch_size, prefetch_count, _global,)
    ...
    def basic_get(self, queue='', no_ack=False):
        ...
        if frame is not None:
            return self.Message(self, frame['properties'], frame['delivery_info'], frame['body'],)

    def basic_consume(self, queue='', consumer_tag=None, no_local=False, no_ack=False, exclusive=False, callback=None, arguments=None, nowait=False):
    ...
    def basic_publish(self, body, exchange='', routing_key='', mandatory=False, immediate=False, **properties):
...
class Connection(_librabbitmq.Connection):
    ...
    def __init__(self, host='localhost', userid='guest', password='guest', virtual_host='/', port=5672, channel_max=0xffff,  frame_max=131072, heartbeat=0, lazy=False, **kwargs):
        if ':' in host:
            host, port = host.split(':')
        super(Connection, self).__init__( hostname=host, port=int(port), userid=userid, password=password, virtual_host=virtual_host, channel_max=channel_max, frame_max=frame_max, heartbeat=heartbeat, )
        self.channels = {}
